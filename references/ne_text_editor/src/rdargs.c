...
/* Copyright (c) University of Cambridge 1990, 1993, 1994 */
...
/* Command line argument decoding. These routine provide a straight-
forward means of decoding a command line which consists of keyword/
value pairs. The style is very much that of the Tripos system. The
externally-visible routine is called rdargs(), and it takes the
following arguments:
...
(4) arg_results *results is a pointer to a vector of structures of type
    ...
    (b) union { int number; char *text } data, which contains either a number,
        for a numerical or switch argument, or a pointer for a string.

    For non-existent arguments the data.number field is set to zero (i.e. FALSE
    for switches and NULL for strings via data.text). The presence field need
...
name of the key. This routine is used only for generating
the name in cases of error. */

static char *findkey(int number, char *keys, char *word)
{
int ch, i;
int j = 0;
int k = 0;
...
and there may be syonyms separated by '='. */

static int findarg(char *keys, char *s)
{
int matching = TRUE;
int argnum = 0;
int i = 0;
...
static void arg_error(arg_result *results, char *arg, char *message)
{
...
static int arg_setup_values(int argc, char **argv, int *a_argindex,
  int *a_argnum, LONGINT argflags, arg_result *results, char *arg,
    int present_value)
{
int argnum = *a_argnum;
int argindex = *a_argindex;
LONGINT argcount = (argflags & 0x00FF0000) >> 16;   /* max args */
...
for (;;)
  ...
  else results[argnum++].data.text = argv[argindex++];
  ...
  nextstring = (unsigned char *)(argv[argindex]);
...
/* This is the procedure that is visible to the outside world. */

int rdargs(int argc, char **argv, char *keystring, arg_result *results)
{
int keyoffset[30];
int argmax = 0;
int argindex = 1;
...
while ((ch = keystring[++i]) != 0)
  ...
  if (ch == '?') results[argmax].presence |= rdargflag_q;
    ...
    default:
    if (isdigit((unsigned int)(keystring[i])))
      {
...
while (argindex < argc)
  ...
  if (arg[0] == '-' && arg[1] != 0)
    ...
    argument value is present if required, and then call the routine which
  ...
  keyed items have been explicitly specified. */
    ...
    {
    int rc;
    LONGINT argflags = 0;    /* keep compiler happy */
    int argnum = -1;
...
for (i = 0; i <= argmax; i++)
  {
  LONGINT argflags = results[i].presence;
  LONGINT p = argflags & argflag_presence_mask;
  if (p == arg_present_not || p == argflag_presence_mask)
    ...
    if ((argflags & rdargflag_a) == 0)
      ...
      for (j = 0; j < 30; j++)
        if (keyoffset[j] == i) { keynumber = j; break; }
      arg_error(results, findkey(keynumber, keystring, (char *)(results+2)),
